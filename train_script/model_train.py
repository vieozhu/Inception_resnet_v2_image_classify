...
dataset_dir = '/path/to/tfrecord files/'
#export_path_base = '/expose/'


# State where your log file is at. If it doesn't exist, create it.
log_dir = '/path/to/log_1'
...
labels_file = dataset_dir + 'label.txt'
...
file_pattern = 'train_%s_*.tfrecord'
...
def get_split(split_name, dataset_dir, file_pattern=file_pattern, file_pattern_for_counting='train'):
    '''
...
def run():
    ...
    with tf.Graph().as_default() as graph:
        ...
        predictions = tf.argmax(end_points['Predictions'], 1, name="Predictions")
        ...
        def train_step(sess, train_op, global_step):
            ...
            logging.info('time:%s, global step %s: loss: %.4f (%.2f sec/step)',
                         time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), global_step_count, total_loss, time_elapsed)
        ...
        with sv.managed_session() as sess:
            ...
            logging.info('Finished training! start Saving model to disk now....')
            sv.saver.save(sess, "./train.ckpt")
